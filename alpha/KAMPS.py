def KAMPS(question):
    ...
    f = []
    doc = ['alpha/textfiles/person.txt', 'alpha/textfiles/team.txt', 'alpha/textfiles/topics.txt']
    dataset = [['alpha/textfiles/person.txt', "people-team-location"], ['alpha/textfiles/team.txt', "team-location"],
               ['alpha/textfiles/topics.txt', "topic-team"]]
    for x in doc:
        f.append(open(x, 'rb').read())

    terms = []
    for x in f:
        terms.append(x.lower().rstrip('\r'))
    ...
    file1 = open("alpha/textfiles/people_db.txt", 'rb')
    file2 = open("alpha/textfiles/team_db.txt", 'rb')
    file3 = open("alpha/textfiles/location_db.txt", 'rb')
    file4 = open("alpha/textfiles/topics_db.txt", 'rb')
        ...
        for j in range(len(queue)):
            ...
            if (i < len(queue) - 1):
                ...
                if (dict[start].connectedTo[dict[end]] == 1):
                    if (not (nodeValue.isdigit()) and (
                            nodeValue in dict[start].hashTable or anotherValue in dict[start].hashTable)):
    ...
    template = {'people-team-location': 'Name who works with team sits on location',
                'team-location': 'team sits on location. Can I help anything else with you?',
                'topic-team': 'Well, team works mainly on topic'}
    print(answer)
    finalAnswer = ''
